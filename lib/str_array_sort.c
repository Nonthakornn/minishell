...
static char	**copy_arr(char **str)
{
	...
	arr_len = count_str_arr(str);
	copy_arr = new_str_arr(arr_len + 1);
	i = 0;
	while (str && str[i])
	{
...
char	**sort_str_arr(char **str_arr)
{
	char	**sort_arr;
	int		arr_len;
	int		min_index;
	...
	arr_len = count_str_arr(str_arr);
	sort_arr = new_str_arr(arr_len + 1);
	...
	while (i < arr_len)
	{
		...
		temp = rm_str_arr(temp, min_index);
		i++;
	}
	free(temp);
	return (sort_arr);
